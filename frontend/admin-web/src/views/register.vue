    ...
    <el-form ref="registerRef" :model="registerForm" :rules="registerRules" class="register-form">
      ...
      <el-form-item style="width:100%;">
        ...
        >
          ...
          <span v-else>{{ $t('common.registering') }}</span>
        </el-button>
        <div style="float: right;">
          <router-link class="link-type" :to="'/login'">{{ $t('register.goLogin') }}</router-link>
        </div>
      </el-form-item>
    </el-form>
    <!--  底部  -->
...
const registerRules = {
  username: [
    ...
    { min: 2, max: 20, message: t('user.usernameLengthBetween'), trigger: "blur" }
  ],
  password: [
    { required: true, trigger: "blur", message: t('register.passwordRequired') },
    { min: 5, max: 20, message: t('user.passwordLengthBetween'), trigger: "blur" },
  ...
  proxy.$refs.registerRef.validate(valid => {
    if (valid) {
      ...
      register(registerForm.value).then(res => {
        ...
        ElMessageBox.alert(`<font color='red'>${t('register.registerSuccess', { username })}</font>`, t('common.systemTip'), {
