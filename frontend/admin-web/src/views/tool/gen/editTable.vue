    ...
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane :label="$t('gen.columnInfo')" name="columnInfo">
        <el-table ref="dragTable" :data="columns" row-key="columnId" :max-height="tableHeight">
          <el-table-column :label="$t('gen.serialNumber')" type="index" min-width="5%" class-name="allowDrag"/>
          <el-table-column :label="$t('gen.columnName')" prop="columnName" min-width="10%" :show-overflow-tooltip="true" class-name="allowDrag"/>
          ...
          <el-table-column :label="$t('gen.edit')" min-width="5%">
            <template #default="scope">
          ...
          <el-table-column :label="$t('gen.required')" min-width="5%">
            <template #default="scope">
          ...
          <el-table-column :label="$t('gen.htmlType')" min-width="12%">
            <template #default="scope">
              <el-select v-model="scope.row.htmlType">
                <el-option :label="$t('gen.input')" value="input" />
...
function submitForm() {
  ...
  Promise.all([basicForm, genForm].map(getFormPromise)).then(res => {
    ...
    if (validateResult) {
      ...
      proxy.$modal.msgError(t('gen.formValidationFailed'))
    }
  })
}
