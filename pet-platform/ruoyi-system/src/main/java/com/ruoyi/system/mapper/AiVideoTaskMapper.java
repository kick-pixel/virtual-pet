 ...
 * @author ruoyi
 */
public interface AiVideoTaskMapper
{
    /**
     * 查询AI视频任务
     *
     ...
     * @return 结果
     */
    public int updateVideoTaskStatus(@Param("taskId") Long taskId, @Param("status") String status, @Param("progress") Integer progress);
     ...
     * @return 结果
     */
    public int updateVideoTaskError(@Param("taskId") Long taskId, @Param("errorCode") String errorCode, @Param("errorMessage") String errorMessage);
     ...
     * @return 结果
     */
    public int incrementLikeCount(Long taskId);
     ...
     * @return 结果
     */
    public int decrementLikeCount(Long taskId);
    ...
    /** 管理后台视频列表查询（含 adminStatus / userId / 时间范围过滤） */
    List<AiVideoTask> selectAdminVideoList(AiVideoTask query);

    /** 更新管理状态（封禁/解封） */
    int updateAdminStatus(@Param("taskId") Long taskId, @Param("adminStatus") Integer adminStatus);

    /** 浏览量增量更新（Redis 批量落库专用） */
    int incrementViewCount(@Param("taskId") Long taskId, @Param("delta") Long delta);
}
